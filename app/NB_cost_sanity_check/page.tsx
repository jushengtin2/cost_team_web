...
export default function NB_CostSanityCheckPage() {
  ...
  const handleDelete = async () => {  //要多寫一個跳出頁面的話也要刪掉資料
    try {
        const response = await fetch('http://taiwan-cost-team:8080/NB_delete', {
            method: 'POST',
            mode: 'cors',
            headers: {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch('http://taiwan-cost-team:8080/NB_upload', { //測試點
        method: 'POST',
        mode: 'cors',
        body: formData,
      });
  ...
  const handle_BOM_Cost_Check = async () => {
    ...
    try {
      const response = await fetch('http://taiwan-cost-team:8080/NB_bom_cost_check', {
        method: 'GET',
        mode: 'cors',
      });
  ...
  const handle_HQM_based_component_Check = async () => {
    ...
    setLoading(true); // 顯示旋轉動畫
    try {
      const response = await fetch('http://taiwan-cost-team:8080/NB_hqm_based_component_check', {
        method: 'GET',
        mode: 'cors',
      });
  ...
  const handle_BOM_based_component_Check = async() => {
    ...
    setLoading(true); // 顯示旋轉動畫
    try {
      const response = await fetch('http://taiwan-cost-team:8080/NB_bom_based_component_check', {
        method: 'GET',
        mode: 'cors',
      });
